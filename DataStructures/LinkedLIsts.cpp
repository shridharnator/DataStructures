...
public:
	...
	void appendNode(Node* n) {
		if (nodeExists(n->key) != NULL) {
			cout << "Node already exists" << std::endl;
	...
	void prependNode(Node* n) {
		if (nodeExists(n->key) != NULL) {
			cout << "Node Already exists with key value :" << n->key << "Appenend another node with different key value" << endl;
		}
		else {
			n->next = head;
			head = n;
	...
	//Insert a node after a particular node in the lsit
